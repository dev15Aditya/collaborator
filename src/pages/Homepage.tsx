...
const Homepage = () => {
    ...
    const cardChip = (title: string, icon: string, index: number) => {
        return (
            <Link key={index} to={title.replace(" ", "").toLowerCase()} className="bg-white border p-6 rounded-xl shadow-lg hover:shadow-2xl transition duration-300 hover:scale-105 transform flex flex-col items-center justify-center">
